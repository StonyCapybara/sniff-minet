...
export default function Clueboard() {
  const [mainOpen, setMainOpen]: [boolean, Dispatch<SetStateAction<boolean>>] = useState(true);
  return <main className="w-fit flex justify-center items-center m-auto mt-12 pb-12">
    <div className="bg-lblue flex-[1] border-4 border-dblue rounded-3xl dblue-shadow w-fit m-auto">
      ...
      {mainOpen
        ? <button onClick={() => { setMainOpen(false) }}><Image src={"/clueboard.png"} alt="" width={1185} height={706} className="mx-8 mb-8" /></button>
        : <button onClick={() => { setMainOpen(true) }}><Image src={"/cluepost.png"} alt="" width={1185} height={706} className="mx-8 mb-8" /></button>}
    </div>
  </main>
}